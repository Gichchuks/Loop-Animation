jQuery(function($) {
	...
	function ContactF() {
		if (GoogleMaps.length) {
			var customMapType = new google.maps.StyledMapType(JSON.parse(GoogleMaps.attr("data-style")));
			var customMapTypeId = 'custom_style';

			Map = new google.maps.Map(GoogleMaps[0], {
				zoom: 12,
				center: {
					lat: parseFloat(GoogleMaps.attr("data-latitude")),
					lng: parseFloat(GoogleMaps.attr("data-longtitude"))
				},
				scrollwheel: false,
			...
			var Marker = new google.maps.Marker({
				...
				position: {lat: parseFloat(GoogleMaps.attr("data-latitude")), lng: parseFloat(GoogleMaps.attr("data-longtitude"))},
				map: Map
			});
				...
				Intro.find(".IntroBtnWrapper").animate({opacity:0}, 300, function() {
					...
					IntroBtnProgress.animate({opacity:0}, 600, function() {
						...
						Intro.animate({opacity:0}, 600, function() {
							...
							google.maps.event.trigger(GoogleMaps[0], 'resize');
							Map.setCenter(new google.maps.LatLng(parseFloat(GoogleMaps.attr("data-latitude")), parseFloat(GoogleMaps.attr("data-longtitude"))));
							Intro.remove();
						});
					});
				});
